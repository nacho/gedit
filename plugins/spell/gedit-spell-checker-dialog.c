...
struct _GeditSpellCheckerDialog 
{
	...
	gchar			*mispelled_word;

	GtkWidget 		*mispelled_word_label;
	...
	*/
	...
	signals [CHANGE] = 
		g_signal_new ("change",
			      ...
			      NULL, NULL,
			      gedit_marshal_BOOLEAN__STRING_STRING,
			      G_TYPE_BOOLEAN, 
			      2, 
			      G_TYPE_STRING,
			      G_TYPE_STRING);
...
gedit_spell_checker_dialog_init (GeditSpellCheckerDialog *dlg)
{
	...
	dlg->mispelled_word_label = glade_xml_get_widget (gui, "mispelled_word_label");
...
gedit_spell_checker_dialog_destroy (GtkObject *object)
{
	...
	if (dlg->mispelled_word != NULL)
	{
		g_free (dlg->mispelled_word);
		dlg->mispelled_word = NULL;
	}
...
gedit_spell_checker_dialog_set_spell_checker (GeditSpellCheckerDialog *dlg, GeditSpellChecker *spell)
{
	...
	if (dlg->mispelled_word != NULL)
		gedit_spell_checker_dialog_set_mispelled_word (dlg, dlg->mispelled_word, -1);
	else
...
void
gedit_spell_checker_dialog_set_mispelled_word (GeditSpellCheckerDialog *dlg, 
		const gchar* word, gint len)
{
	...
	g_return_if_fail (!gedit_spell_checker_check_word (dlg->spell_checker, word, -1, NULL));

	/* build_suggestions_list */
	if (dlg->mispelled_word != NULL)
		g_free (dlg->mispelled_word);

	dlg->mispelled_word = g_strdup (word);
	
	tmp = g_strdup_printf("<b>%s</b>", word);
	gtk_label_set_label (GTK_LABEL (dlg->mispelled_word_label), tmp);
	g_free (tmp);

	sug = gedit_spell_checker_get_suggestions (dlg->spell_checker,
		       dlg->mispelled_word, -1, &error);
...
check_word_button_clicked_handler (GtkButton *button, GeditSpellCheckerDialog *dlg)
{
	...
	g_return_if_fail (g_utf8_strlen (word, -1) > 0);
	
	if (gedit_spell_checker_check_word (dlg->spell_checker, word, -1, NULL))
	{
		GtkListStore *store;
		GtkTreeIter iter;
...
add_word_button_clicked_handler (GtkButton *button, GeditSpellCheckerDialog *dlg)
{
	...
	g_signal_emit (G_OBJECT (dlg), signals [ADD_WORD_TO_PERSONAL], 0, 
			word, &handler_ret);
...
ignore_button_clicked_handler (GtkButton *button, GeditSpellCheckerDialog *dlg)
{
	...
	g_signal_emit (G_OBJECT (dlg), signals [IGNORE], 0, 
			word, &handler_ret);
...
ignore_all_button_clicked_handler (GtkButton *button, GeditSpellCheckerDialog *dlg)
{
	...
	g_signal_emit (G_OBJECT (dlg), signals [IGNORE_ALL], 0, 
			word, &handler_ret);
