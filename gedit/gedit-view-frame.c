...
#include "theatrics/gedit-view-container.h"
#include "theatrics/gedit-rounded-frame.h"
...
start_interactive_search_real (GeditViewFrame *frame)
{
	...
	gedit_view_container_add_animated_widget (GEDIT_VIEW_CONTAINER (frame->priv->container),
                       ...
                       GeditViewFrame *frame)
{
	if (frame->priv->search_widget != NULL)
	{
		...
		gtk_widget_get_allocation (frame->priv->search_widget,
		                           &widget_alloc);
		gedit_view_container_move_widget (GEDIT_VIEW_CONTAINER (frame->priv->container),
		                                  frame->priv->search_widget,
...
gedit_view_frame_init (GeditViewFrame *frame)
{
	...
	frame->priv->container = gedit_view_container_new (frame->priv->view);
	gtk_widget_show (frame->priv->container);

	/* Create the scrolled window */
	sw = gtk_scrolled_window_new (NULL, NULL);

	gtk_container_add (GTK_CONTAINER (sw), frame->priv->container);
